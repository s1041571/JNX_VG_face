...
var img2 = document.getElementById("camera");

can.width = img.width;
can.height = img.height;

var viewArea = SVG().addTo('#imageMap').size('100%', '100%');
viewArea.attr('id', 'svgArea_view'); //新增頁畫面
var drawArea = SVG().addTo('#imageMap2').size('100%', '100%');
drawArea.attr('id', 'svgArea'); //設定圍籬頁畫面
...
function resize(area_coord_array) {
    ...
    drawArea.clear(); 
    viewArea.clear(); 
    var new_coordStr = "", new_coordStr2 = "";
    area_coord_array.forEach(coord => {
        ...
        for (var coord_group of coord_list) {
            if(coord_group=="") continue
                ...
                new_coordStr2 += ((coord[0]/100*img2.width).toFixed(0)) +','+((coord[1]/100*img2.height).toFixed(0)) +' ';
            }
        }
        drawSVG(new_coordStr,drawArea)//因畫布比例變動，須更新畫布上座標
        drawSVG(new_coordStr2,viewArea)//因畫布比例變動，須更新畫布上座標
        new_coordStr = "", new_coordStr2 = "";
    });
...
function drawSVG(crood, drawObj) {
    ...
    if (drawObj.node.id == 'svgArea' || drawObj.node.id == 'svgArea_view') {
...
function saveAreaCoord() {
    ...
    if(fence_enable==true && areas_points =='')   //確認啟用狀態要有座標資訊
        ...
        $.ajax({
            type: 'POST',
            async: false,
            url: "/process/save_fence_coord",
            contentType: 'application/json; charset=UTF-8',
            data: JSON.stringify(json_data),
            success: function (result) {
                Swal.fire({ title: result, text:'儲存成功，將重新載入圍籬...',icon: 'success',showConfirmButton: false, timer: 1500});
                // $('#vf-modal').modal('hide')
