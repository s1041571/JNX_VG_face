...
$('.list-group li').on('click', function (event) {
    if ($(this).html().indexOf('head') > -1)
        $('#class_img').attr("src", "../static/img/body_head.png");
    if ($(this).html().indexOf('hand') > -1)
        $('#class_img').attr("src", "../static/img/body_hand.png");
    if ($(this).html().indexOf('body') > -1)
        $('#class_img').attr("src", "../static/img/body.png");
    if ($(this).html().indexOf('foot') > -1)
        $('#class_img').attr("src", "../static/img/body_feet.png");
...
function saveAreaCoord() {
    ...
    // must have fence
    if(areas_points.length == 0){
        swal({
            title: "請確認虛擬圍籬！", 
            text: "啟用虛擬圍籬，請記得框選圍籬區域再進行儲存", 
            icon: "warning"
        })
    }
    else{
...
function getAlarmGroup(){
    var channel_list = [];
    var mailGroup ="";
    var log_save_day = parseInt($('#input_log_save_day').val());
    if($("#check_mail").prop("checked") == true) {
        if($('#mailGroupSelect')[0].selectedIndex == 0){
            swal({ title: "請記得選取收件者群組！" ,icon: 'info', timer: 1500});
            return ""
        }
        else{
